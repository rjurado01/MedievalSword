...
public class SquarePath extends Image {
	...
	public SquarePath( Vector2 square_position, boolean attainable, boolean selected ) {
		if( selected )
			setSelected( square_position, attainable );
		else
	 ...
	 */
	private void setSelected( Vector2 square_position, boolean attainable ) {
		...
		if( attainable )
			setRegion( Assets.getTextureRegion( "squareBlueSelected" ) );
		else
			setRegion( Assets.getTextureRegion( "squareRedSelected" ) );
	}
